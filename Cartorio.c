...
int registro() // função responsável por cadastrar os usuários no sistema.
{
	...
	char sobrenome[40];
	char cargo[40];
	//Final criuação das variáveis/string.
	
	printf ("Digite o CPF a ser cadastrado: "); //coletando informações do usuário.
	scanf("%s", cpf); //%s refere-se a string.
	
	strcpy(arquivo, cpf);  // responsavel por copia valores da sstrings
...
int deletar()
{
	...
	if(file == NULL)
	{
		printf("O Usuário na se encontra no sistema! ** Deletado com sucesso **.\n");
		system("pause");
	}
